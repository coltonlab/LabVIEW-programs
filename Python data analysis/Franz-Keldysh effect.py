...
def plot_franz_keldysh(data, title, flip = False, reflection=False, smooth=False, Eg=0, Ex=0):
    # plots the EA or ER
    fig, ax1 = plt.subplots()
    fig2, ax2 = plt.subplots()

    # pcg.plot_EA_series(data, ax=ax1, color_map_name='plasma_r', smooth=smooth, energy=True, phased=True, flip=flip)
    pcg.plot_EA_amplitude(data, ax=ax1, color_map_name='plasma_r', smooth=smooth, energy=True, phased=True, flip=flip)
    # I now have an array for each voltage with the peaks after plot_EA_amplitude since it saves it to the same spot as the data
    pcg.plot_peak_amplitude(data, ax=ax2, color_map_name='plasma_r')
...
def C5_2022_09_08():
    ...
    plot_franz_keldysh(data810, title='C5 EA analysis', flip = True, reflection=False, Eg=2.845, Ex=2.845-272/1000, smooth=False)
