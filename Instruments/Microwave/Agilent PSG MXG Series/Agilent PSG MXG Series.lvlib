<?xml version='1.0' encoding='UTF-8'?>
<Library LVVersion="9008000">
	<Property Name="Instrument Driver" Type="Str">True</Property>
	<Property Name="NI.Lib.Description" Type="Str">LabVIEW Plug and Play instrument driver for Agilent PSG MXG Series RF signal generator. For more information about this driver, please refer to Agilent PSG MXG Series Readme.html.</Property>
	<Property Name="NI.Lib.HelpPath" Type="Str"></Property>
	<Property Name="NI.Lib.Icon" Type="Bin">#1#!!!!!!!)!"1!&amp;!!!-!%!!!@````]!!!!"!!%!!!*/!!!*Q(C=\&gt;1R&lt;NN!%)8B:S/&amp;+Q'[1;!O^6R"KAUDU27G3[X7J7ZA4/=5;81#!TJ!CMQ66+@4&amp;:3@QZ&amp;"Q)$6R%!-G043YNP&gt;W5]E2;GW'_G44J?WW;M&lt;`?/9/A[(/KF^.MY`D?=6HW?=2UTHV\$HHD\NAL-O/JP7HSQY1&lt;R9`Y*@LWY@`2`^\\P`QM;9_YO$XHC4LEJ+3VL1H'&lt;V\5Z*HO2*HO2*HO2"(O2"(O2"(O2/\O2/\O2/\O2'&lt;O2'&lt;O2'&lt;O3VEYN=Z#,HKC3,*QMF2:-#S7!I3A[**`%EHM4$K2*0YEE]C3@R-%3**`%EHM34?*CGR*.Y%E`C34S5+EH74IYH]6"?A3@Q"*\!%XB95I%H!!3,"97$)D!5&gt;!9@!E`A#4R]6/!*0)%H]!1?OB6Y!E`A#4S"BSFV6;)UQU[/BT*S0)\(]4A?RU.J/2\(YXA=D_.B/4E?R_-AH!7&gt;YB$E4()'/#?/R`(Q4Y\(]4A?R_.Y[+I\Z(6F"MWQE_-R0)&lt;(]"A?QU-*'2\$9XA-D_'BL!S0Y4%]BM@QM*1-D_%R0!&lt;%7*4F:21T*BK$D-$Q]&amp;&gt;0C^6&gt;CJ*9\@X4("^5^1/I@L$5$YT[16$@907.5^]1^9678U$VB6&amp;`9@585106#[M,KA@KS0&amp;!W^.WN#VN1VP4FL1&amp;&lt;4Z-`==$D]?D$I?$^PO^&gt;LO&gt;NNON.JO.VOOVFMOF&amp;IO&amp;ZP0Z]^PK-`OY8:X@3`&gt;]@PKTOPVZ]`XO[@?XB]=PK[]`@KVOB`QNW`B?_B`?D&lt;L7;@+;ZRL^"1!V2`5!!!!!</Property>
	<Property Name="NI.Lib.Version" Type="Str">1.4.0.0</Property>
	<Item Name="Public" Type="Folder">
		<Property Name="NI.LibItem.Scope" Type="Int">1</Property>
		<Item Name="Action-Status" Type="Folder">
			<Item Name="Abort.vi" Type="VI" URL="../Public/Action-Status/Abort.vi"/>
			<Item Name="Action-Status.mnu" Type="Document" URL="../Public/Action-Status/Action-Status.mnu"/>
			<Item Name="Apply Two Tone Settings.vi" Type="VI" URL="../Public/Action-Status/Apply Two Tone Settings.vi"/>
			<Item Name="Disable All Digital Modulation.vi" Type="VI" URL="../Public/Action-Status/Disable All Digital Modulation.vi"/>
			<Item Name="Disable Markers.vi" Type="VI" URL="../Public/Action-Status/Disable Markers.vi"/>
			<Item Name="Enable Synchronization Trigger Receiving.vi" Type="VI" URL="../Public/Action-Status/Enable Synchronization Trigger Receiving.vi"/>
			<Item Name="Initiate Synchronization Trigger.vi" Type="VI" URL="../Public/Action-Status/Initiate Synchronization Trigger.vi"/>
			<Item Name="Initiate.vi" Type="VI" URL="../Public/Action-Status/Initiate.vi"/>
			<Item Name="Query Amp Sweep Start.vi" Type="VI" URL="../Public/Action-Status/Query Amp Sweep Start.vi"/>
			<Item Name="Query Amp Sweep Stop.vi" Type="VI" URL="../Public/Action-Status/Query Amp Sweep Stop.vi"/>
			<Item Name="Query Amplitude Mode.vi" Type="VI" URL="../Public/Action-Status/Query Amplitude Mode.vi"/>
			<Item Name="Query Amplitude.vi" Type="VI" URL="../Public/Action-Status/Query Amplitude.vi"/>
			<Item Name="Query Freq Sweep Start.vi" Type="VI" URL="../Public/Action-Status/Query Freq Sweep Start.vi"/>
			<Item Name="Query Freq Sweep Stop.vi" Type="VI" URL="../Public/Action-Status/Query Freq Sweep Stop.vi"/>
			<Item Name="Query Frequency Mode.vi" Type="VI" URL="../Public/Action-Status/Query Frequency Mode.vi"/>
			<Item Name="Query Frequency.vi" Type="VI" URL="../Public/Action-Status/Query Frequency.vi"/>
			<Item Name="Query Output.vi" Type="VI" URL="../Public/Action-Status/Query Output.vi"/>
			<Item Name="Query Sweep Dwell.vi" Type="VI" URL="../Public/Action-Status/Query Sweep Dwell.vi"/>
			<Item Name="Query Sweep Points.vi" Type="VI" URL="../Public/Action-Status/Query Sweep Points.vi"/>
			<Item Name="Reset Digital Signal Interface Module.vi" Type="VI" URL="../Public/Action-Status/Reset Digital Signal Interface Module.vi"/>
			<Item Name="Reset IQ DAC.vi" Type="VI" URL="../Public/Action-Status/Reset IQ DAC.vi"/>
			<Item Name="Reset Reference Oscillator.vi" Type="VI" URL="../Public/Action-Status/Reset Reference Oscillator.vi"/>
			<Item Name="Save Recall Sweep List.vi" Type="VI" URL="../Public/Action-Status/Save Recall Sweep List.vi"/>
			<Item Name="Select Waveform.vi" Type="VI" URL="../Public/Action-Status/Select Waveform.vi"/>
			<Item Name="Send Software Trigger.vi" Type="VI" URL="../Public/Action-Status/Send Software Trigger.vi"/>
			<Item Name="Set Amp Sweep Start.vi" Type="VI" URL="../Public/Action-Status/Set Amp Sweep Start.vi"/>
			<Item Name="Set Amp Sweep Stop.vi" Type="VI" URL="../Public/Action-Status/Set Amp Sweep Stop.vi"/>
			<Item Name="Set Amplitude Mode.vi" Type="VI" URL="../Public/Action-Status/Set Amplitude Mode.vi"/>
			<Item Name="Set Amplitude.vi" Type="VI" URL="../Public/Action-Status/Set Amplitude.vi"/>
			<Item Name="Set Freq Sweep Start.vi" Type="VI" URL="../Public/Action-Status/Set Freq Sweep Start.vi"/>
			<Item Name="Set Freq Sweep Stop.vi" Type="VI" URL="../Public/Action-Status/Set Freq Sweep Stop.vi"/>
			<Item Name="Set Frequency Mode.vi" Type="VI" URL="../Public/Action-Status/Set Frequency Mode.vi"/>
			<Item Name="Set Frequency Reference.vi" Type="VI" URL="../Public/Action-Status/Set Frequency Reference.vi"/>
			<Item Name="Set Frequency.vi" Type="VI" URL="../Public/Action-Status/Set Frequency.vi"/>
			<Item Name="Set Output.vi" Type="VI" URL="../Public/Action-Status/Set Output.vi"/>
			<Item Name="Set Phase Reference.vi" Type="VI" URL="../Public/Action-Status/Set Phase Reference.vi"/>
			<Item Name="Set Sweep Dwell.vi" Type="VI" URL="../Public/Action-Status/Set Sweep Dwell.vi"/>
			<Item Name="Set Sweep Points.vi" Type="VI" URL="../Public/Action-Status/Set Sweep Points.vi"/>
			<Item Name="Single Sweep.vi" Type="VI" URL="../Public/Action-Status/Single Sweep.vi"/>
			<Item Name="Wait for Operation Complete.vi" Type="VI" URL="../Public/Action-Status/Wait for Operation Complete.vi"/>
		</Item>
		<Item Name="Configure" Type="Folder">
			<Item Name="Modulation" Type="Folder">
				<Item Name="Digital Modulation" Type="Folder">
					<Item Name="Configure IQ Adjustment.vi" Type="VI" URL="../Public/Configure/Modulation/Digital Modulation/Configure IQ Adjustment.vi"/>
					<Item Name="Configure IQ Adjustment External.vi" Type="VI" URL="../Public/Configure/Modulation/Digital Modulation/Configure IQ Adjustment External.vi"/>
					<Item Name="Configure IQ Attenuation.vi" Type="VI" URL="../Public/Configure/Modulation/Digital Modulation/Configure IQ Attenuation.vi"/>
					<Item Name="Configure IQ Correction.vi" Type="VI" URL="../Public/Configure/Modulation/Digital Modulation/Configure IQ Correction.vi"/>
					<Item Name="Configure IQ Skew Correction.vi" Type="VI" URL="../Public/Configure/Modulation/Digital Modulation/Configure IQ Skew Correction.vi"/>
					<Item Name="Calibrate IQ.vi" Type="VI" URL="../Public/Configure/Modulation/Digital Modulation/Calibrate IQ.vi"/>
					<Item Name="Digital Modulation.mnu" Type="Document" URL="../Public/Configure/Modulation/Digital Modulation/Digital Modulation.mnu"/>
				</Item>
				<Item Name="Custom Modulation" Type="Folder">
					<Item Name="Configure Burst Shape.vi" Type="VI" URL="../Public/Configure/Modulation/Custom Modulation/Configure Burst Shape.vi"/>
					<Item Name="Configure Data Pattern.vi" Type="VI" URL="../Public/Configure/Modulation/Custom Modulation/Configure Data Pattern.vi"/>
					<Item Name="Configure Data Clock.vi" Type="VI" URL="../Public/Configure/Modulation/Custom Modulation/Configure Data Clock.vi"/>
					<Item Name="Custom Modulation.mnu" Type="Document" URL="../Public/Configure/Modulation/Custom Modulation/Custom Modulation.mnu"/>
				</Item>
				<Item Name="Dual Modulation" Type="Folder">
					<Item Name="Configure Multicarrier Setup.vi" Type="VI" URL="../Public/Configure/Modulation/Dual Modulation/Configure Multicarrier Setup.vi"/>
					<Item Name="Dual Modulation.mnu" Type="Document" URL="../Public/Configure/Modulation/Dual Modulation/Dual Modulation.mnu"/>
				</Item>
				<Item Name="Dual ARB" Type="Folder">
					<Item Name="Configure ARB.vi" Type="VI" URL="../Public/Configure/Modulation/Dual ARB/Configure ARB.vi"/>
					<Item Name="Configure Marker RF Blanking.vi" Type="VI" URL="../Public/Configure/Modulation/Dual ARB/Configure Marker RF Blanking.vi"/>
					<Item Name="Configure ARB Scaling.vi" Type="VI" URL="../Public/Configure/Modulation/Dual ARB/Configure ARB Scaling.vi"/>
					<Item Name="Configure ARB Clipping.vi" Type="VI" URL="../Public/Configure/Modulation/Dual ARB/Configure ARB Clipping.vi"/>
					<Item Name="Configure Multiple Baseband Generator Synchronization.vi" Type="VI" URL="../Public/Configure/Modulation/Dual ARB/Configure Multiple Baseband Generator Synchronization.vi"/>
					<Item Name="Dual ARB.mnu" Type="Document" URL="../Public/Configure/Modulation/Dual ARB/Dual ARB.mnu"/>
				</Item>
				<Item Name="Two Tone And Multitone" Type="Folder">
					<Item Name="Configure Multiple Tone.vi" Type="VI" URL="../Public/Configure/Modulation/Two Tone And Multitone/Configure Multiple Tone.vi"/>
					<Item Name="Configure Multiple Tone Characteristics.vi" Type="VI" URL="../Public/Configure/Modulation/Two Tone And Multitone/Configure Multiple Tone Characteristics.vi"/>
					<Item Name="Configure Two Tone.vi" Type="VI" URL="../Public/Configure/Modulation/Two Tone And Multitone/Configure Two Tone.vi"/>
					<Item Name="Two Tone And Multitone.mnu" Type="Document" URL="../Public/Configure/Modulation/Two Tone And Multitone/Two Tone And Multitone.mnu"/>
				</Item>
				<Item Name="AWGN Real-Time" Type="Folder">
					<Item Name="Configure RT AWGN.vi" Type="VI" URL="../Public/Configure/Modulation/AWGN Real-Time/Configure RT AWGN.vi"/>
					<Item Name="AWGN Real-Time.mnu" Type="Document" URL="../Public/Configure/Modulation/AWGN Real-Time/AWGN Real-Time.mnu"/>
				</Item>
				<Item Name="AWGN ARB" Type="Folder">
					<Item Name="Configure AWGN ARB.vi" Type="VI" URL="../Public/Configure/Modulation/AWGN ARB/Configure AWGN ARB.vi"/>
					<Item Name="AWGN ARB.mnu" Type="Document" URL="../Public/Configure/Modulation/AWGN ARB/AWGN ARB.mnu"/>
				</Item>
				<Item Name="Configure Modulation.vi" Type="VI" URL="../Public/Configure/Modulation/Configure Modulation.vi"/>
				<Item Name="Configure Modulation (Amplitude).vi" Type="VI" URL="../Public/Configure/Modulation/Configure Modulation (Amplitude).vi"/>
				<Item Name="Configure Modulation (Frequency).vi" Type="VI" URL="../Public/Configure/Modulation/Configure Modulation (Frequency).vi"/>
				<Item Name="Configure Modulation (Phase).vi" Type="VI" URL="../Public/Configure/Modulation/Configure Modulation (Phase).vi"/>
				<Item Name="Configure Modulation (Pulse).vi" Type="VI" URL="../Public/Configure/Modulation/Configure Modulation (Pulse).vi"/>
				<Item Name="Configure Modulation (IQ).vi" Type="VI" URL="../Public/Configure/Modulation/Configure Modulation (IQ).vi"/>
				<Item Name="Configure Modulation (Custom).vi" Type="VI" URL="../Public/Configure/Modulation/Configure Modulation (Custom).vi"/>
				<Item Name="Configure Modulation (Digital).vi" Type="VI" URL="../Public/Configure/Modulation/Configure Modulation (Digital).vi"/>
				<Item Name="Configure Modulation (Wideband IQ).vi" Type="VI" URL="../Public/Configure/Modulation/Configure Modulation (Wideband IQ).vi"/>
				<Item Name="Configure Low Frequency Output.vi" Type="VI" URL="../Public/Configure/Modulation/Configure Low Frequency Output.vi"/>
				<Item Name="Configure Modulation Source.vi" Type="VI" URL="../Public/Configure/Modulation/Configure Modulation Source.vi"/>
				<Item Name="Configure Modulation Waveform.vi" Type="VI" URL="../Public/Configure/Modulation/Configure Modulation Waveform.vi"/>
				<Item Name="Configure Modulation Waveform (Noise).vi" Type="VI" URL="../Public/Configure/Modulation/Configure Modulation Waveform (Noise).vi"/>
				<Item Name="Configure Modulation Waveform (Ramp).vi" Type="VI" URL="../Public/Configure/Modulation/Configure Modulation Waveform (Ramp).vi"/>
				<Item Name="Configure Modulation Waveform (Dual-Sine).vi" Type="VI" URL="../Public/Configure/Modulation/Configure Modulation Waveform (Dual-Sine).vi"/>
				<Item Name="Configure Modulation Waveform (Swept-Sine).vi" Type="VI" URL="../Public/Configure/Modulation/Configure Modulation Waveform (Swept-Sine).vi"/>
				<Item Name="Configure Filter.vi" Type="VI" URL="../Public/Configure/Modulation/Configure Filter.vi"/>
				<Item Name="Configure Trigger.vi" Type="VI" URL="../Public/Configure/Modulation/Configure Trigger.vi"/>
				<Item Name="Configure External Trigger.vi" Type="VI" URL="../Public/Configure/Modulation/Configure External Trigger.vi"/>
				<Item Name="Configure IQ Output.vi" Type="VI" URL="../Public/Configure/Modulation/Configure IQ Output.vi"/>
				<Item Name="Configure Reference.vi" Type="VI" URL="../Public/Configure/Modulation/Configure Reference.vi"/>
				<Item Name="Configure Sample Clock Rate.vi" Type="VI" URL="../Public/Configure/Modulation/Configure Sample Clock Rate.vi"/>
				<Item Name="Configure Marker Polarity.vi" Type="VI" URL="../Public/Configure/Modulation/Configure Marker Polarity.vi"/>
				<Item Name="Configure Marker Destination.vi" Type="VI" URL="../Public/Configure/Modulation/Configure Marker Destination.vi"/>
				<Item Name="Configure RT Noise.vi" Type="VI" URL="../Public/Configure/Modulation/Configure RT Noise.vi"/>
				<Item Name="Configure RT Phase Noise Impairment.vi" Type="VI" URL="../Public/Configure/Modulation/Configure RT Phase Noise Impairment.vi"/>
				<Item Name="Configure Baseband Frequency Offset.vi" Type="VI" URL="../Public/Configure/Modulation/Configure Baseband Frequency Offset.vi"/>
				<Item Name="Configure VCO Clock.vi" Type="VI" URL="../Public/Configure/Modulation/Configure VCO Clock.vi"/>
				<Item Name="Modulation.mnu" Type="Document" URL="../Public/Configure/Modulation/Modulation.mnu"/>
			</Item>
			<Item Name="Digital Signal Interface" Type="Folder">
				<Item Name="Configure Digital Signal Interface Module.vi" Type="VI" URL="../Public/Configure/Digital Signal Interface/Configure Digital Signal Interface Module.vi"/>
				<Item Name="Configure Digital Clock.vi" Type="VI" URL="../Public/Configure/Digital Signal Interface/Configure Digital Clock.vi"/>
				<Item Name="Configure Digital Data.vi" Type="VI" URL="../Public/Configure/Digital Signal Interface/Configure Digital Data.vi"/>
				<Item Name="Configure Digital Output Data.vi" Type="VI" URL="../Public/Configure/Digital Signal Interface/Configure Digital Output Data.vi"/>
				<Item Name="Configure Digital IQ Data.vi" Type="VI" URL="../Public/Configure/Digital Signal Interface/Configure Digital IQ Data.vi"/>
				<Item Name="Digital Signal Interface.mnu" Type="Document" URL="../Public/Configure/Digital Signal Interface/Digital Signal Interface.mnu"/>
			</Item>
			<Item Name="Configure Frequency.vi" Type="VI" URL="../Public/Configure/Configure Frequency.vi"/>
			<Item Name="Configure Frequency Channel.vi" Type="VI" URL="../Public/Configure/Configure Frequency Channel.vi"/>
			<Item Name="Configure Reference Oscillator.vi" Type="VI" URL="../Public/Configure/Configure Reference Oscillator.vi"/>
			<Item Name="Configure Reference Oscillator Auto Source.vi" Type="VI" URL="../Public/Configure/Configure Reference Oscillator Auto Source.vi"/>
			<Item Name="Configure Power.vi" Type="VI" URL="../Public/Configure/Configure Power.vi"/>
			<Item Name="Configure ALC.vi" Type="VI" URL="../Public/Configure/Configure ALC.vi"/>
			<Item Name="Configure ALC Search.vi" Type="VI" URL="../Public/Configure/Configure ALC Search.vi"/>
			<Item Name="Configure RF Output.vi" Type="VI" URL="../Public/Configure/Configure RF Output.vi"/>
			<Item Name="Configure Sweep Characteristics.vi" Type="VI" URL="../Public/Configure/Configure Sweep Characteristics.vi"/>
			<Item Name="Configure Sweep Trigger.vi" Type="VI" URL="../Public/Configure/Configure Sweep Trigger.vi"/>
			<Item Name="Configure Sweep.vi" Type="VI" URL="../Public/Configure/Configure Sweep.vi"/>
			<Item Name="Configure Sweep (List).vi" Type="VI" URL="../Public/Configure/Configure Sweep (List).vi"/>
			<Item Name="Configure Sweep (Step).vi" Type="VI" URL="../Public/Configure/Configure Sweep (Step).vi"/>
			<Item Name="Configure Sweep (Ramp).vi" Type="VI" URL="../Public/Configure/Configure Sweep (Ramp).vi"/>
			<Item Name="Configure Waveform Sweep.vi" Type="VI" URL="../Public/Configure/Configure Waveform Sweep.vi"/>
			<Item Name="Configure Marker.vi" Type="VI" URL="../Public/Configure/Configure Marker.vi"/>
			<Item Name="Configure.mnu" Type="Document" URL="../Public/Configure/Configure.mnu"/>
		</Item>
		<Item Name="Utility" Type="Folder">
			<Item Name="Error Query.vi" Type="VI" URL="../Public/Utility/Error Query.vi"/>
			<Item Name="Reset.vi" Type="VI" URL="../Public/Utility/Reset.vi"/>
			<Item Name="Revision Query.vi" Type="VI" URL="../Public/Utility/Revision Query.vi"/>
			<Item Name="Self-Test.vi" Type="VI" URL="../Public/Utility/Self-Test.vi"/>
			<Item Name="Query Instrument Information.vi" Type="VI" URL="../Public/Utility/Query Instrument Information.vi"/>
			<Item Name="Utility.mnu" Type="Document" URL="../Public/Utility/Utility.mnu"/>
		</Item>
		<Item Name="Close.vi" Type="VI" URL="../Public/Close.vi"/>
		<Item Name="Initialize.vi" Type="VI" URL="../Public/Initialize.vi"/>
		<Item Name="VI Tree.vi" Type="VI" URL="../Public/VI Tree.vi"/>
		<Item Name="dir.mnu" Type="Document" URL="../Public/dir.mnu"/>
	</Item>
	<Item Name="Private" Type="Folder">
		<Property Name="NI.LibItem.Scope" Type="Int">2</Property>
		<Item Name="Default Instrument Setup.vi" Type="VI" URL="../Private/Default Instrument Setup.vi"/>
		<Item Name="Extended User Data.ctl" Type="VI" URL="../Private/Extended User Data.ctl"/>
		<Item Name="Extended User Data.vi" Type="VI" URL="../Private/Extended User Data.vi"/>
		<Item Name="Model Recognition.vi" Type="VI" URL="../Private/Model Recognition.vi"/>
		<Item Name="Option Recognition.vi" Type="VI" URL="../Private/Option Recognition.vi"/>
		<Item Name="Send Command.vi" Type="VI" URL="../Private/Send Command.vi"/>
		<Item Name="Send Query.vi" Type="VI" URL="../Private/Send Query.vi"/>
		<Item Name="Series Recognition.vi" Type="VI" URL="../Private/Series Recognition.vi"/>
	</Item>
	<Item Name="Agilent PSG MXG Series Readme.html" Type="Document" URL="../Agilent PSG MXG Series Readme.html"/>
</Library>
